...
let messageModal //  message modal for 5 points
...
gameState = {
    ...
    gameOver: 2,
    //message modal state: shows message at 5 pts
    message: 3
}
//background
bg = {
...
score = {
    ...
    render: function() {
        if (gameState.current == gameState.play ||
            ...
            //check if score reached 5 to trigger message
            if (this.current == 5 && !messageModal.hasShown) {
                gameState.current = gameState.message
                messageModal.hasShown = true
            }
...
bird = {
    animation: [
        {imgX: 278, imgY: 113},  //  position 0
        {imgX: 276, imgY: 142},  //  position 1
        {imgX: 278, imgY: 173},  //  position 2
        {imgX: 276, imgY: 142}   //  position 1
    ],
    fr: 0,
    //object's key-value properties pinpointing its location
...
//message modal object
messageModal = {
    //message chunks split into 3 sentences per page
    chunks: [
        "Hello! Happy 1st monthsary! I can hardly believe it's already been a month of us sharing this journey together. Every day with you feels new and exciting, from the little moments we laugh over to the things we experience side by side. I still smile every time I see the penguin pillow or the keychain you gave me, they're not just gifts, they're reminders of your thoughtfulness and the warmth you bring into my life.",
        "Thank you for letting me be your first in so many ways, for trusting me, and for sharing these special experiences. Sure, we've had our small bumps along the way, but honestly, they just make our story feel more real and meaningful. I'm grateful for every part of this journey with you, and I'm looking forward to all the months to come.",
        "Here's to us, to laughter, to little adventures, and to everything that's still ahead. Always yours, Kelvin"
    ],
    currentPage: 0,
    hasShown: false,
    ...
    nextPage: function() {
        if (this.currentPage < this.chunks.length - 1) {
            this.currentPage++
        } else {
            //after last page, resume game
    ...
    render: function() {
        if (gameState.current == gameState.message) {
            //draw semi-transparent background overlay
            ctx.fillStyle = 'rgba(0, 0, 0, 0.7)'
            ctx.fillRect(0, 0, cvs.width, cvs.height)
            
            //draw white rectangle for message box
            const boxX = 20
            const boxY = 100
            const boxWidth = cvs.width - 40
            const boxHeight = 280
            ...
            const lineHeight = 16
            let currentY = boxY + padding + 20
            
            const text = this.chunks[this.currentPage]
            ...
            const pageText = (this.currentPage + 1) + ' / ' + this.chunks.length
            ctx.fillText(pageText, cvs.width/2, boxY + boxHeight - 8)
            
            //draw continue hint
            ctx.fillStyle = '#999999'
            ctx.font = '10px Arial'
            const hintText = 'Click or press spacebar to continue'
            ctx.fillText(hintText, cvs.width/2, boxY + boxHeight + 20)
        }
    }
}
/************************
...
cvs.addEventListener('click', () => {
    //if message state >> advance to next page
    if (gameState.current == gameState.message) {
        messageModal.nextPage()
        return
    }
